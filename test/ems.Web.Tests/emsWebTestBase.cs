...
{
    public abstract class emsWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static emsWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(emsWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<emsDbContext> action)
        {
            using (var context = IocManager.Resolve<emsDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<emsDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<emsDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<emsDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<emsDbContext, Task<T>> func)
        {
            T result;
